...
function validateAndParseOptions(args: UserOptions) {
  ...
  assert.ok(
    availableSources.includes(args.source),
    `Invalid value for --source: ${args.source}, must be one of ${availableSources.join(', ')}`,
  );

  assert.ok(semver.lt(args.from, args.to), '--from cannot be greater than --to');

  const testFilePath = path.isAbsolute(args.test) ? args.test : path.join(process.cwd(), args.test);

  // assert file exists
  assert.ok(fs.existsSync(testFilePath), `Test file does not exist:${args.test} (full path: ${testFilePath})`);
